...
// 上記テストコードの共通処理をまとめて書く！
// まとめることで、テストクラス全体やテストメソッドの見通しがよくなるというメリットがある！
// 今回は、全てのテストメソッドで共通する、テスト対象クラス（Calculatorクラス）のインスタンス化の部分を共通化する！

class CalculatorTest {
//	テスト対象クラスをフィールドに追加
//	これで、各テストメソッドからCalculatorクラスを参照できる！
	private Calculator calc;
	
//	各テストメソッドの実行前に毎回呼ばれる処理を記載する！（@BeforeEachアノテーション）
	@BeforeEach
	void setUp() {
